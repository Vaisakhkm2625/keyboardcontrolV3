...
class Manager:
    ...
    def __init__(self, ui):
        ...
        self.ui.actionremove_item.triggered.connect(
            self.on_remove_item_pressed)

        self.ui.actionadd_event.triggered.connect(self.on_add_event_pressed)
        self.ui.actionremove_event.triggered.connect(
            self.on_remove_event_pressed)
    ...
    def load_plugins(self):
        ...
        self.plugin_manager = PluginManager(
            categories_filter={"Action": Action, "Event": Event}, plugin_info_ext="kb-plugin")
        ...
        for action_plugin in self.plugin_manager.getPluginsOfCategory("Action"):
            ...
            action_plugin.ui_index = self.ui.actions_area.addWidget(
                ui_widget())
    ...
    def set_item_ui_action(self, item: Item):
        action_plugin = self.plugin_manager.getPluginByName(
            item.action_type, category='Action')

        logger.debug(f"action {action_plugin} ui for {item.name}")

        try:
            self.ui.actions_area.widget(
                action_plugin.ui_index).set_ui_data(item.action_data)
        except Exception as e:
            logger.debug(
                f"unable to set data for action plugin data for item {item.name}")
    ...
    def set_item_ui_events(self, item: Item):
        ...
        for event in item.event_list:
            ...
            ui_widget_class = self.plugin_manager.getPluginByName(
                event["type"], category='Event').plugin_object.get_ui_widget()
            ...
            # Set the size hint for proper sizing
            event_list_item.setSizeHint(ui_widget.sizeHint())
    ...
    def apply_changes(self):
        ...
        action_plugin = self.plugin_manager.getPluginByName(
            item.action_type, category='Action')
        item.action_data = self.ui.actions_area.widget(
            action_plugin.ui_index).get_ui_data()
    ...
    def on_add_item_pressed(self):
        ...
        action_list = []
        action_list = [
            action_plugin.name for action_plugin in self.plugin_manager.getPluginsOfCategory("Action")]
    ...
    def on_add_event_pressed(self):
        ...
        event_plugin_list = []
        event_plugin_list = [
            event_plugin.name for event_plugin in self.plugin_manager.getPluginsOfCategory("Event")]

        add_event_dialog = AddEventDialog(event_list=event_plugin_list)
        add_event_dialog.data_passed_signal.connect(
            self.add_event_to_curr_item_list)
    ...
    def run_events(self):
        ...
        for event_plugin in event_plugins:
            event_plugin.plugin_object.set_data_mappings(
                id_mappings=self.group_by_event(event_plugin.name))
            event_plugin.plugin_object.stop_event_listener()
            event_plugin.plugin_object.start_event_listener(
                self.event_callback)
        # event_plugin.plugin_object.stop_event_listener()
...
class MainWindow(QMainWindow):
    ...
    def add_style_menu(self):
        ...
        color_actions = [QAction(color.replace(
            "_", "-").replace(".xml", ""), self) for color in colors]
